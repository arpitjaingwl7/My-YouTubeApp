import React, { useEffect,useState } from 'react'
import { useDispatch } from 'react-redux'
import { toggleMenu,setQuerry } from '../utils/appSlice';
import {Link} from "react-router-dom"
import { serchSugetionApi } from '../utils/constants';

const Head = () => {

   
   let[isFocus,setIsFocus]=useState(false)
   let [searchQuery,setSearchQuery]=useState("");
   let [suggestions,setSugestions]=useState([]);

   let handleSearch=()=>{
      // console.log("handle ")
      dispatch(setQuerry(searchQuery))
   }
   console.log(searchQuery)
    useEffect(()=>{


     let timer= setTimeout(()=>{
         getSearchSugestions();
      }
      ,350)
   return ()=>{
      clearTimeout(timer)
   }
   
   }
      

    
    ,[searchQuery])

const getSearchSugestions=async()=>{


   
   // let data=await fetch(serchSugetionApi+searchQuery)
   let data=await fetch("https://youtube-backend-server.onrender.com/?query="+searchQuery)
   let json=await data.json();
   // console.log(json[1]);

   
   setSugestions(json[1])


}

   let dispatch=useDispatch();
  return (
   <div className='w-84 md:w-full'>
    <div className=' grid grid-flow-col  m-2 p-4  shadow'>
    
     <div className='flex col-span-1 '>
       
        <img className='h-7 cursor-pointer' onClick={()=>{
         dispatch(toggleMenu())
        }}  src="data:image/png;base64," alt="ham icon" />
       <Link to="/" onClick={()=>{
         setSearchQuery("");
        dispatch(setQuerry(""))
        
       }}> <img className='h-7 ml-2 cursor-pointer' src="https://upload.wikimedia.org/wikipedia/commons/thumb/b/b8/YouTube_Logo_2017.svg/768px-YouTube_Logo_2017.svg.png" alt="" /></Link>
     </div>

     <div className='col-span-10 px-10 '>
        <input value={searchQuery} onFocus={()=>{
         setIsFocus(true);
        }} onBlur={()=>{
         setTimeout(()=>{
            setIsFocus(false)
         },200)
         
        }} onChange={(e)=>{
         setSearchQuery(e.target.value)
        }} placeholder='Search' type="text" className='border border-gray-400 px-5 p-1 rounded-l-full  w-1/2 '/>
        <button  onClick={handleSearch} className='border bg-gray-100 border-gray-400 px-2 p-1 rounded-r-full'>🔍</button>
         
        {isFocus&&suggestions.length>0&&(<div className='px-2 py-2 fixed w-[28rem] rounded-lg shadow-lg bg-white border border-gray-100'> 
        <ul>
        
        {
         suggestions.map((ele)=>{
            return  <li  onClick={()=>{
               // alert("click")
               // e.preventDefault()
               // e.stopPropagation();
              
               // console.log(555)
               setSearchQuery(ele)
               handleSearch();
            }} key={ele} className='px-2 py-2 shadow-sm hover:bg-gray-200 '>🔍 {ele}</li>
         })
        }
      </ul>
     </div>)}
     </div>

     <div>
        <img className='h-7' src="data:image/png;base64," alt="" />
     </div>

      

    </div>
    </div>
  )
}




export default Head
