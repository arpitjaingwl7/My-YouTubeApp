import React from "react";

const commentData = [
  {
    name: "Arpit",
    text: "lorem Ipsumfsegr ywgdbfb dfhgxdx",
    reply: [
      {
        name: "Arpit",
        text: "lorem Ipsum fsegrywg dbfbdfhgxdx",
        reply: [
          {
            name: "Arpit",
            text: "lorem Ipsumfse grywgdbfbd fhgxdx",
            reply: [],
          }
        ],
      },
      
    ],
  },
  {
    name: "Arpit",
    text: "loremI psumfsegr ywgdbf bdfhgxdx",
    reply: [
      {
        name: "Arpit",
        text: "loremIpsu mfsegryw gdbfbdf hgxdx",
        reply: [],
      },
      {
        name: "Arpit",
        text: "loremIp sumfsegryw gdbfbdfhgxdx",
        reply: [],
      },
    ],
  },
  {
    name: "Arpit",
    text: "lorem Ipsumfs egrywgdb fbdfhgxdx",
    reply: [],
  }
];



const Comment=({obj})=>{

const {name,text,reply}=obj;
return (


  <>
  <div className="flex  shadow-sm mt-2 p-2 rounded-lg bg-gray-100 items-center">
    <div>
   <img className='w-8 h-8 rounded-lg'  src="data:image/png;base64," alt="" />
  </div>
  <div className="ml-2">

   <p className="font-bold">{name}</p>
   <p>{text}</p>

   </div>



  </div>

  <div className="">
  {
    reply.map((ele)=>{
      
      return  <div className=" ml-5 pl-5 border border-l-black"> <Comment obj={ele}></Comment> </div>
    })
  }
</div>

  </>
)

  
}


const CommentsContainer = () => {
  return (
    <div className="mx-5 w-2/3">
      <h1 className="text-2xl font-bold">Comments:</h1>
      
      <div>
        {
          commentData.map((ele)=>{

               return <Comment obj={ele}></Comment>
          })
        }
      
      </div>
    </div>
  );
};

export default CommentsContainer;
